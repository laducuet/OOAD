...
<?php
include("header.php");
date_default_timezone_set('Asia/Karachi'); // Set the default timezone to Pakistan (GMT+5)
            ...
            <div class="product-info">
			  ...
			  <div class="product-gallery-thumbnails">
    			<ol class="thumbnails-list list-unstyled">
        			<?php
        				if (is_array($arr_product_image)) {
							for ($iimg = 0; $iimg < count($arr_product_image); $iimg++) {
...
<div id="divbidstatus">
  ...
  function enableBidForm() {
    var startingTime = new Date("<?php echo date("M d, Y H:i:s", strtotime($rsproduct[8])); ?>").getTime();
    ...
    if (startingTime === currentTime) {
        bidAmountDiv.style.display = "block"; // Show the bid amount div
        purchaseAmountInput.removeAttribute("disabled"); // Enable the input field
    } else {
        bidAmountDiv.style.display = "none"; // Hide the bid amount div
        purchaseAmountInput.setAttribute("disabled", "disabled"); // Disable the input field
    }
  }
</script>
 <br>
