...
$sqlwinningbid = "SELECT winners.*,product.product_id, product.product_name,product.starting_bid, product.ending_bid, product.end_date_time, bidding.bidding_amount,customer.customer_id as cid, customer.customer_name as cname FROM product LEFT JOIN winners ON product.product_id = winners.product_id LEFT JOIN bidding ON bidding.product_id=product.product_id LEFT JOIN customer ON customer.customer_id = bidding.customer_id WHERE product.ending_bid=bidding.bidding_amount AND product.status='Active' AND product.end_date_time<'$dt $tim' AND winners.product_id IS NULL";

$qsqlwinningbid  = mysqli_query($con, $sqlwinningbid);
while ($rswinningbid = mysqli_fetch_array($qsqlwinningbid)) {

	$cid = $rswinningbid["cid"];
	$cname = $rswinningbid["cname"];
	$bidding_amount = $rswinningbid["bidding_amount"];
	$product_id = $rswinningbid["product_id"];
	$product_name = $rswinningbid["product_name"];
	$starting_bid = $rswinningbid["starting_bid"];
	$ending_bid = $rswinningbid["ending_bid"];
	$end_date_time = $rswinningbid["end_date_time"];

	// $data = array($rswinningbid[0], $rswinningbid[23], $rswinningbid[6], $rswinningbid[26]);
	...
	$myBlockchain->addBlock(new Block($currentTimestamp, json_encode($data)));
	...
	// new query after adding blockchain 
	$sqlwinner = "INSERT INTO winners(product_id,customer_id,winning_bid,end_date,status) VALUES('$product_id','$cid','$bidding_amount','$end_date_time','Pending')";
										...
										?>
											...
											<li>
												<a href="deposit.php">
													...
													<div class="item-total">PKR<?php echo $accbalamt; ?></div>
												</a>
											</li>
										<?php
										} else {
						...
						<div class="row">
							<div class="col-lg-12 d-none d-lg-block">
								...
								<div class="main-menu-area">
									<nav>
										<ul>
											<li><a href="view_blockchain.php" style="color:black;">Winners Blockchain</a></li>
